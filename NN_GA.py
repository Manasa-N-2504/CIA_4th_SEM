...
class GA:
    def __init__(self, model, pop_size, mut_rate, decay_rate, x, y):
        self.model = model
    ...
    def primary_population(self):
        return [self.model.get_weights() for _ in range(self.pop_size)]

    def select(self, scores):
        return np.searchsorted(np.cumsum(scores), np.random.uniform(0, np.sum(scores)))

    def crossover(self, male, female):#mKE IR S CROSS
        pt = np.random.randint(1, len(male))
        return male[:pt] + female[pt:], female[:pt] + male[pt:]

    def decay_mut_rate(self):
        self.mut_rate -= self.decay_rate * self.mut_rate

    def mutate(self, child):
        return [weight + np.random.normal(0, 0.1, weight.shape) if np.random.uniform(0, 1) < self.mut_rate else weight for weight in child]

    def generate_offspring(self, scores):
        self.population = [self.mutate(child1) + self.mutate(child2)
                           for child1, child2 in [self.crossover(self.population[self.select(scores)],
...
genetic_optimizer = GA(model, pop_size=20, mut_rate=0.3, decay_rate=0.05, x=x_train, y=y_train)
